...
export const CmsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Fetch initial content from backend API if available
  useEffect(() => {
    const fetchRemoteData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (remoteContent && remoteContent.brand) {
            const normalized = normalizeSiteContent(remoteContent);
            setContent(normalized);
            localStorage.setItem(STORAGE_KEY_CONTENT, JSON.stringify(normalized));
          }
      ...
      try {
        ...
        if (resB.ok) {
          const remoteBookings = await resB.json();
    ...
    setContent((prev) => {
      ...
      try {
        ...
        fetch('/api/content', {
          ...
          // Local storage is primary client backup
        });
      } catch (err) {
